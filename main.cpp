...
int main()
{
    ...
    ffmpeg_handle(avformat_alloc_output_context2(&fmtCtx, NULL, NULL, OUT_FILE_NAME));

    AVStream *vedio_stream = avformat_new_stream(fmtCtx, NULL);
    ...
    codeCtx->width = 640;
    codeCtx->height = 360;
    codeCtx->codec_id = fmtCtx->oformat->video_codec;
    ...
    //打开文件.写入文件头.文件尾需要
    ffmpeg_handle(avio_open(&fmtCtx->pb, OUT_FILE_NAME, AVIO_FLAG_READ_WRITE));
    ...
    while (1) {
        ...
        while (ret >= 0) {
            ...
            ffmpeg_handle(av_write_frame(fmtCtx, packet));
